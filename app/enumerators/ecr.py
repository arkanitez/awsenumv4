...
def enumerate(session: boto3.Session, account_id: str, region: str, g: Graph, warnings: List[str]) -> None:
    ...
    while True:
        ...
        for r in resp.get('repositories', []) or []:
            name = r.get('repositoryName'); arn = r.get('repositoryArn')
            ...
            try:
                ...
                scan_on_push = any((ru.get('scanFrequency') or '').upper() in {'SCAN_ON_PUSH','CONTINUOUS_SCAN'} for ru in rules)
            except Exception:
                pass
            # encryption (repository-level)
            enc_type = (r.get('encryptionConfiguration') or {}).get('encryptionType') or 'AES256'
            kms_key = (r.get('encryptionConfiguration') or {}).get('kmsKey')

            node_id = mk_id('ecr', account_id, region, name)
            g.add_node(node_id, name, 'ecr_repo', region, details={
                'repositoryName': name, 'Arn': arn,
                'scanOnPush': scan_on_push,
                'encryptionType': enc_type, 'kmsKey': kms_key
            })
            if kms_key:
                key_id = kms_key.split('/')[-1]
                kms_id = mk_id('kms', account_id, region, key_id)
                g.add_edge(mk_id('edge', account_id, region, 'ecr-kms', name, key_id),
