...
def enumerate(session: boto3.Session, account_id: str, region: str, g: Graph, warnings: List[str]) -> None:
    ...
    try:
        recs = cfg.describe_configuration_recorders().get("ConfigurationRecorders", []) or []
        status = cfg.describe_configuration_recorder_status().get("ConfigurationRecordersStatus", []) or []
        chans = cfg.describe_delivery_channels().get("DeliveryChannels", []) or []
    ...
    for r in recs:
        ...
        details = {
            ...
            "deliveryChannel": {
                "s3BucketName": chan.get("s3BucketName"),
                "s3KeyPrefix": chan.get("s3KeyPrefix"),
                "snsTopicARN": chan.get("snsTopicARN"),
            },
            # delivery channel KMS key (if any)
            "KmsKeyId": chan.get("kmsKeyArn") or chan.get("kmsKeyId") or chan.get("KmsKeyArn"),
        }
        g.add_node(node_id, f"ConfigRecorder:{name}", "config_recorder", region, details=details)

        # Edges: to S3 bucket if set, to KMS if set
        b = chan.get("s3BucketName")
        if b:
            s3_id = mk_id("s3", account_id, region, b)
            g.add_edge(
                mk_id("edge", account_id, region, "config-s3", name, b),
                node_id,
                s3_id,
                "delivers to",
                "delivery",
        ...
        if kms_arn:
            ...
            g.add_edge(
                mk_id("edge", account_id, region, "config-kms", name, key_id),
                node_id,
                kms_id,
                "encrypts with",
                "kms",
