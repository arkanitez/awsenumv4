...
def enumerate(session: boto3.Session, account_id: str, region: str, g: Graph, warnings: List[str]) -> None:
    ...
    for d in dets:
        try:
            desc = gd.get_detector(DetectorId=d)
        except Exception:
            desc = {}
        node_id = mk_id('guardduty', account_id, region, d)
        g.add_node(node_id, f'GuardDuty:{region}', 'guardduty_detector', region, details={
            'DetectorId': d, 'Status': desc.get('Status'), 'status': desc.get('Status'),
