...
def enumerate(session: boto3.Session, account_id: str, region: str, g: Graph, warnings: List[str]) -> None:
    ...
    for t in trails:
        name = t.get("Name")
        home = t.get("HomeRegion") or region
        if not name:
            continue
        try:
        ...
        details = {
            ...
            "Status": "ENABLED" if st.get("IsLogging") else "DISABLED",
        }
        ...
        if lg_arn:
            lg_name = lg_arn.split(":log-group:")[-1]
            lg_id = mk_id("cwl", account_id, home, lg_name)
            g.add_edge(
                mk_id("edge", account_id, home, "trail-cwl", name, lg_name),
                node_id,
                lg_id,
                "logs to",
                "trail_logs",
        ...
        if kms_arn:
            ...
            g.add_edge(
                mk_id("edge", account_id, home, "trail-kms", name, key_id),
                node_id,
                kms_id,
                "encrypts with",
                "kms",
